...
<p align="center">
  <img src="https://img.shields.io/badge/version-v0.4.0-4ADE80?style=flat-square&labelColor=0a0f14" alt="Version" />
...
- 屈服/疲劳安全系数、安全破裂转速
- 额定/可用/最大储能、比能量
- 疲劳寿命估算（S-N + Goodman/Gerber/Soderberg 修正准则可选）
- 热应力分析（温度降额 + 径向温度梯度）
- 安全评估状态（通过/未通过）
...
- 保存当前方案（参数+材料+计算结果快照）
- 方案列表管理（重命名/删除/清空）
- 对比表格：横向展示 2~4 个方案的关键指标
- 雷达图：多维度可视化对比（比能量/安全系数/储能/质量/惯量/应力）
- 方案数据持久化（localStorage 跨会话保持）

### 参数边界约束
- 几何参数实时校验（内径/外径比、轮毂径合理性、厚度范围）
- 工况参数校验（转速顺序、安全系数合理性）
- NumberInput 组件支持 error（红色）/ warning（黄色）状态
- 参数建议提示文本

### 材料库增强
- 自定义材料编辑器：7 个属性字段（密度/杨氏模量/泊松比/屈服/抗拉/疲劳/热膨胀）
- 自定义材料持久化存储
- material_override 机制：自定义材料直接传入后端计算

### 数据导出
- CSV / JSON / SVG / 参数 JSON 多种格式导出
- **HTML 工程报告**：自包含暗色主题报告，含参数表+结果表+内联SVG应力图+安全评估
- 支持自定义保存路径
...
- 数字输入框长按快速微调
- 实时计算（300ms 防抖）
- **参数撤销/重做**：Ctrl+Z / Ctrl+Y，最多 20 步历史
- **快捷键**：Ctrl+L 切换左栏 / Ctrl+R 切换右栏
...
| 计算核心 | Rust (flyforge-core) — 108 项单元测试 |
| 安装包 | NSIS |
...
```bash
cargo test --workspace    # Rust 单元测试（108 项）
```
...
│   │   ├── ComparePanel.tsx    # 方案对比面板（列表+表格+雷达图）
│   │   ├── RadarChart.tsx      # 雷达图组件
│   │   └── ResultsPanel.tsx    # 右侧结果面板（含疲劳准则选择）
│   ├── services/                 # API 调用层
│   ├── store/                    # 全局状态管理
│   │   ├── index.tsx             # 主状态（含撤销/重做）
│   │   └── schemes.tsx           # 方案管理状态
...
│   │   ├── fatigue.rs            # 疲劳寿命估算（S-N + Goodman/Gerber/Soderberg）
│   │   ├── thermal.rs            # 温度场热应力分析
...
│   │   ├── dynamics.rs           # 动力学仿真
│   │   ├── solver.rs             # 计算调度器
│   │   └── types.rs              # 核心数据类型 + 12 种材料库 + material_override
